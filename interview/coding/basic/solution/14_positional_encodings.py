...
def sinusoidal_table(max_len, d_model, base=10000.0):
    ...
    angles = pos / (base ** (i / d_model))                                         # (L, D/2)
...
def rope_cos_sin(positions, head_dim, base=10000.0):
    ...
    inv_freq = base ** (-torch.arange(0, head_dim, 2, dtype=torch.float32) / head_dim)   # (D/2,)
    angle = positions.to(torch.float32).unsqueeze(-1) * inv_freq                           # (..., D/2)
    angle = torch.cat([angle, angle], dim=-1)                                               # (..., D)
...
def apply_rope(x, positions, base=10000.0):
    ...
    """
    D = x.shape[-1]
    cos, sin = rope_cos_sin(positions, D, base)           # (..., D) broadcastable to x
