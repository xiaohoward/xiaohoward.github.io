...
def shard_bounds(numel, world_size):
    ...
    """
    S = math.ceil(numel / world_size) if numel > 0 else 0
    return [(min(r * S, numel), min((r + 1) * S, numel)) for r in range(world_size)]


def flatten(tensors):
    """Concatenate tensors (any shapes, same dtype) into one contiguous 1-D tensor, in order. Returns a new tensor."""
    return torch.cat([t.reshape(-1) for t in tensors]) if tensors else torch.empty(0)
...
def unflatten(flat, like):
    ...
    for t in like:
        ...
        off += n
    assert off == flat.numel(), "flat length does not match shapes"
    return out
...
def ring_allreduce(bufs, comm):
    ...
    M = bufs[0].numel()
    C = math.ceil(M / N)
    x = [torch.cat([b.clone(), b.new_zeros(N * C - M)]).view(N, C) for b in bufs]
    if N == 1:
        return [x[0].reshape(-1)[:M].clone()]
    for s in range(N - 1):
        incoming = []
        for r in range(N):
            c = (r - s) % N
            incoming.append(comm.send(r, (r + 1) % N, x[r][c]))
        for r in range(N):
            src = (r - 1) % N
            c = (src - s) % N
            x[r][c] += incoming[src]
    for s in range(N - 1):
        ...
        for r in range(N):
            ...
            c = (src + 1 - s) % N
            x[r][c] = incoming[src]
    return [xr.reshape(-1)[:M].clone() for xr in x]
...
class Zero1Adam:
    ...
    def step(self, rank_grads):
        ...
        """
        N = self.world_size
        avg = bucketed_allreduce_mean(rank_grads, self.comm)
        self.step_count += 1
        b1, b2 = self.betas
        t = self.step_count
        shards = []
        for r in range(N):
            s, e = self.bounds[r]
            flat_p = flatten(self.rank_params[r])
            g = flatten(avg[r])[s:e]
            p = flat_p[s:e].clone()
            self.m[r].mul_(b1).add_(g, alpha=1 - b1)
            self.v[r].mul_(b2).addcmul_(g, g, value=1 - b2)
            denom = (self.v[r] / (1 - b2 ** t)).sqrt().add_(self.eps)
            p.addcdiv_(self.m[r] / (1 - b1 ** t), denom, value=-self.lr)
            shards.append(p)
        for r in range(N):
            flat_p = flatten(self.rank_params[r])
            for src in range(N):
                s, e = self.bounds[src]
                if src == r:
                    flat_p[s:e] = shards[src]
                elif e > s:
                    flat_p[s:e] = self.comm.send(src, r, shards[src])
            for dst, view in zip(self.rank_params[r], unflatten(flat_p, self.rank_params[r])):
                dst.copy_(view)
