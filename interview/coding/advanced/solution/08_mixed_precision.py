...
class DynamicLossScaler:
    ...
    def unscale_(self, grads_fp16):
        ...
        """
        inv = 1.0 / self.scale
        grads = [g.float() * inv for g in grads_fp16]
        found_inf = any(bool(torch.isinf(g).any() or torch.isnan(g).any()) for g in grads)
...
def mixed_precision_step(master, batch, scaler, opt_state, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
    ...
    """
    p16 = [p.detach().to(torch.float16).requires_grad_(True) for p in master]
    loss = forward_loss(p16, batch)
    scaler.scale_loss(loss).backward()
    grads16 = [p.grad for p in p16]
    if grad_hook is not None:
        grads16 = grad_hook(grads16)
    grads32, found_inf = scaler.unscale_(grads16)
    if not found_inf:
        adamw_update_(master, grads32, opt_state, lr=lr, betas=betas, eps=eps, weight_decay=weight_decay)
    scaler.update(found_inf)
    return float(loss.item()), found_inf, grads32
