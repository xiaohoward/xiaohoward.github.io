...
def ddpm_pred_to_x0_eps(pred, x_t, alphas_cumprod_t, kind):
    ...
    """
    ab = _bcast(alphas_cumprod_t, x_t)
    a, b = ab.sqrt(), (1 - ab).sqrt()
    if kind == "eps":
        return (x_t - b * pred) / a, pred
    if kind == "x0":
        return pred, (x_t - a * pred) / b
    if kind == "v":
        return a * x_t - b * pred, b * x_t + a * pred
...
def weighted_mse(pred, target, weight):
    ...
    return (weight.to(per_sample.dtype) * per_sample).mean()
