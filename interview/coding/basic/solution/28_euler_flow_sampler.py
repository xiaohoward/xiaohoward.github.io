...
def make_timesteps(n_steps, shift=1.0):
    ...
    """
    t = torch.linspace(1.0, 0.0, n_steps + 1, dtype=torch.float32)
...
def sample_euler(model, x1, timesteps):
    ...
    for i in range(len(timesteps) - 1):
        t_cur, t_next = timesteps[i].item(), timesteps[i + 1].item()
        v = model(x, torch.full((B,), t_cur, dtype=x.dtype))
        x = euler_step(x, v, t_cur, t_next)
    return x
...
def sample_with_x0_clamp(model, x1, timesteps, lo=-1.0, hi=1.0, return_x0_history=False):
    ...
    for i in range(len(timesteps) - 1):
        t_cur, t_next = timesteps[i].item(), timesteps[i + 1].item()
        v = model(x, torch.full((B,), t_cur, dtype=x.dtype))
        x0_hat = torch.clamp(x - t_cur * v, lo, hi)
        eps_hat = (x - (1 - t_cur) * x0_hat) / t_cur
        x = (1 - t_next) * x0_hat + t_next * eps_hat
        hist.append(x0_hat)
    return (x, torch.stack(hist)) if return_x0_history else x
