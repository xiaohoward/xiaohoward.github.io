...
def gaussian_nll_loss(mean, log_std, target, log_std_min=-5.0, log_std_max=2.0):
    ...
    return (0.5 * (target - mean) ** 2 * inv_var + log_std + 0.5 * math.log(2 * math.pi)).mean()
...
def bc_step(policy, opt, obs, actions, head="mse"):
    ...
    out = policy(obs)
    if head == "mse":
        loss = mse_loss(out, actions)
    elif head == "gaussian":
        D = actions.shape[-1]
        loss = gaussian_nll_loss(out[..., :D], out[..., D:], actions)
    else:
...
def flow_matching_loss(net, obs, a0, t, eps):
    ...
    """
    tt = t.view(-1, 1, 1).to(a0.dtype)
    a_t = (1 - tt) * a0 + tt * eps
    v_pred = net(obs, a_t, t)
    return ((v_pred - (eps - a0)) ** 2).mean()
...
def diffusion_policy_step(net, opt, obs, a0, gen):
    ...
    eps = torch.randn(a0.shape, generator=gen)
    loss = flow_matching_loss(net, obs, a0, t, eps)
    opt.zero_grad()
    loss.backward()
    opt.step()
    return loss.item()
...
def sample_actions(net, obs, chunk_shape, K, gen):
    ...
    B = obs.shape[0]
    H, D = chunk_shape
    a = torch.randn((B, H, D), generator=gen)
    dt = 1.0 / K
    for k in range(K):
        t = torch.full((B,), 1.0 - k / K)
        a = a - dt * net(obs, a, t)
    return a
...
def evaluate_success(outcomes):
    ...
    """
    o = torch.as_tensor(outcomes).bool()
    n = int(o.numel())
    k = int(o.sum())
    p, lo, hi = wilson_interval(k, n)
    return {"rate": p, "lo": lo, "hi": hi, "n": n}
