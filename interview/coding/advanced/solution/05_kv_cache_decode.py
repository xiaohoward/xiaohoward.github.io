...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, cache=None, window=None):
        ...
        q, k, v = [z.view(B, T_new, self.n_heads, self.head_dim).transpose(1, 2) for z in (q, k, v)]
        T_past = 0
        ...
        mask = causal_mask(T_new, T_past, window)
        out = F.scaled_dot_product_attention(q, k, v, attn_mask=mask)
        out = self.proj(out.transpose(1, 2).reshape(B, T_new, D))
...
def causal_mask(T_new, T_past, window=None):
    ...
    """
    p = torch.arange(T_past, T_past + T_new)[:, None]
    j = torch.arange(T_past + T_new)[None, :]
    mask = j <= p
    if window is not None:
        mask = mask & (j > p - window)
    return mask
...
def decode_incremental(model, tokens, window=None):
    ...
    with torch.no_grad():
        for t in range(tokens.shape[1]):
            logits, caches = model(tokens[:, t:t + 1], start_pos=t, caches=caches, window=window)
            outs.append(logits)
    return torch.cat(outs, dim=1), caches
