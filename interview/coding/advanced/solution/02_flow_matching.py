...
def flow_matching_loss(model, x0, eps, t, cond):
    ...
    Returns:
        0-d tensor.
    """
    x_t = interpolate(x0, eps, t)
    return torch.mean((model(x_t, t, cond) - velocity_target(x0, eps)) ** 2)
...
def cfg_velocity(model, x, t, cond, null_cond, scale):
    ...
    Returns:
        (B, D) tensor.
    """
    out = model(torch.cat([x, x], 0), torch.cat([t, t], 0), torch.cat([cond, null_cond], 0))
...
def sample_euler(velocity_fn, x1, timesteps):
    ...
    with torch.no_grad():
        for i in range(len(timesteps) - 1):
            ...
            x = x + (t_next - t) * velocity_fn(x, t.expand(B))
    return x
...
def sample_heun(velocity_fn, x1, timesteps):
    ...
    with torch.no_grad():
        for i in range(len(timesteps) - 1):
            t, t_next = timesteps[i], timesteps[i + 1]
            dt = t_next - t
            v1 = velocity_fn(x, t.expand(B))
            x_e = x + dt * v1
            v2 = velocity_fn(x_e, t_next.expand(B))
            x = x + dt * 0.5 * (v1 + v2)
    return x
