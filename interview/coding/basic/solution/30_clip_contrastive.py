...
def clip_loss(img, txt, logit_scale):
    ...
    """
    logits = clip_logits(img, txt, logit_scale)
...
def recall_at_k(img, txt, k):
    ...
    """
    sim = l2_normalize(img) @ l2_normalize(txt).t()          # (B, B)
    B = sim.shape[0]
    y = torch.arange(B)
    top_i2t = sim.topk(k, dim=1).indices                     # (B, k)
    top_t2i = sim.t().topk(k, dim=1).indices
    r_i2t = (top_i2t == y[:, None]).any(1).float().mean()
    r_t2i = (top_t2i == y[:, None]).any(1).float().mean()
    return float(r_i2t), float(r_t2i)
...
def siglip_loss(img, txt, logit_scale, logit_bias):
    ...
    """
    B = img.shape[0]
    z = clip_logits(img, txt, logit_scale) + logit_bias
    y = 2.0 * torch.eye(B, dtype=z.dtype, device=z.device) - 1.0
    return -F.logsigmoid(y * z).sum() / B
