...
def split_sequence(x, n_ranks, zigzag=False):
    ...
    if not zigzag:
        assert S % n_ranks == 0
        c = S // n_ranks
        pos = [torch.arange(r * c, (r + 1) * c) for r in range(n_ranks)]
    else:
        assert S % (2 * n_ranks) == 0
        c = S // (2 * n_ranks)
        pos = [torch.cat([torch.arange(r * c, (r + 1) * c),
                          torch.arange((2 * n_ranks - 1 - r) * c, (2 * n_ranks - r) * c)]) for r in range(n_ranks)]
    return [x[:, :, p] for p in pos], pos
...
def merge_sequence(chunks, positions, S):
    ...
    B, H, _, D = chunks[0].shape
    out = chunks[0].new_empty(B, H, S, D)
...
def ring_attention(q_chunks, k_chunks, v_chunks, positions, causal=False):
    ...
    for r in range(N):
        q = q_chunks[r]
        B, H, Sq, _ = q.shape
        state.append([torch.full((B, H, Sq, 1), -1e30), torch.zeros(B, H, Sq, 1), torch.zeros(B, H, Sq, D)])
    held = list(range(N))            # held[r] = index of the K/V chunk rank r currently holds
    trace = [[] for _ in range(N)]
    for s in range(N):
        for r in range(N):
            src = held[r]
            trace[r].append(src)
            k, v = k_chunks[src], v_chunks[src]
            mask = None
            if causal:
                mask = positions[r][:, None] >= positions[src][None, :]
            state[r] = list(_block_update(*state[r], q_chunks[r], k, v, mask))
        held = [held[(r - 1) % N] for r in range(N)]      # rank r receives from rank r-1
    outs = [acc / l for (_, l, acc) in state]
    return outs, trace
...
def all_to_all_seq_to_heads(chunks):
    ...
    H = chunks[0].shape[1]
    h = H // N
    return [torch.cat([c[:, r * h:(r + 1) * h] for c in chunks], dim=2) for r in range(N)]
...
def all_to_all_heads_to_seq(chunks):
    ...
    S = chunks[0].shape[2]
    c = S // N
    return [torch.cat([hc[:, :, r * c:(r + 1) * c] for hc in chunks], dim=1) for r in range(N)]
...
def ulysses_attention(q_chunks, k_chunks, v_chunks, causal=False):
    ...
    """
    qh = all_to_all_seq_to_heads(q_chunks)
    kh = all_to_all_seq_to_heads(k_chunks)
    vh = all_to_all_seq_to_heads(v_chunks)
    oh = [F.scaled_dot_product_attention(q, k, v, is_causal=causal) for q, k, v in zip(qh, kh, vh)]
    return all_to_all_heads_to_seq(oh)
