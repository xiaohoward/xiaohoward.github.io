...
def next_token_nll(logits, labels, attention_mask=None, ignore_index=-100, label_smoothing=0.0):
    ...
    if attention_mask is not None:
        ...
        valid = valid & am[:, :-1] & am[:, 1:]
    safe_tgt = torch.where(valid, tgt, torch.zeros_like(tgt))
    nll = F.cross_entropy(lg.reshape(-1, V), safe_tgt.reshape(-1), reduction="none",
                          label_smoothing=label_smoothing).view(B, L - 1)
    return nll * valid.to(nll.dtype), valid
...
def sequence_loss(nll, valid):
    ...
    return (nll * v).sum(1) / v.sum(1).clamp_min(1.0)
...
def batch_loss(nll, valid, reduction="token"):
    ...
    if reduction == "token":
        v = valid.to(nll.dtype)
        return (nll * v).sum() / v.sum().clamp_min(1.0)
    if reduction == "sequence":
        return sequence_loss(nll, valid).mean()
...
def perplexity(nll, valid):
    ...
    """
    return float(torch.exp(batch_loss(nll, valid, "token")))
...
def pack_sequences(seqs, block_size, eos_id, pad_id):
    ...
    for d, s in enumerate(seqs):
        ...
        if len(doc) > block_size:
            raise ValueError(f"document {d} has {len(doc)} tokens (incl. eos) > block_size {block_size}")
        if len(cur_tok) + len(doc) > block_size:
            blocks.append(cur_tok + [pad_id] * (block_size - len(cur_tok)))
            ids.append(cur_id + [-1] * (block_size - len(cur_id)))
            cur_tok, cur_id = [], []
        cur_tok += doc
        cur_id += [d] * len(doc)
    if cur_tok:
        blocks.append(cur_tok + [pad_id] * (block_size - len(cur_tok)))
        ids.append(cur_id + [-1] * (block_size - len(cur_id)))
    return torch.tensor(blocks, dtype=torch.long), torch.tensor(ids, dtype=torch.long)
...
def packed_causal_mask(sequence_ids):
    ...
    """
    L = sequence_ids.shape[1]
    causal = torch.tril(torch.ones(L, L, dtype=torch.bool))
    same = sequence_ids[:, :, None] == sequence_ids[:, None, :]
    not_pad = (sequence_ids != -1)[:, :, None]
    return causal[None] & same & not_pad
