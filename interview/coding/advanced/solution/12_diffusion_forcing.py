...
def causal_frame_mask(F):
    ...
    return torch.tril(torch.ones(F, F, dtype=torch.bool))
...
def diffusion_forcing_loss(model, x0, gen):
    ...
    """
    B, F, D = x0.shape
    t = torch.rand(B, F, generator=gen)
    eps = torch.randn(B, F, D, generator=gen)
    x_t = (1 - t)[..., None] * x0 + t[..., None] * eps
    v = eps - x0
    v_hat = model(x_t, t, causal_frame_mask(F))
...
def sample_autoregressive(model, B, F, D, n_steps, gen, teacher_frames=None):
    ...
    """
    out = torch.zeros(B, F, D)
    for i in range(F):
        context = teacher_frames[:, :i] if teacher_frames is not None else out[:, :i]
        K, V = model.context_kv(context)
        x = torch.randn(B, 1, D, generator=gen)
        for k in range(n_steps):
            t_k = 1.0 - k / n_steps
            v = model.denoise_with_context(x, torch.full((B, 1), t_k), K, V)
            x = x - v / n_steps
        out[:, i] = x[:, 0]
    return out
