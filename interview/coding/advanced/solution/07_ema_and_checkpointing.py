...
class EMA:
    ...
    def get_decay(self):
        ...
        if self.warmup:
            n = self.num_updates
            return min(self.decay, (1 + n) / (10 + n))
        return self.decay
    ...
    def restore(self, model):
        ...
        if self.backup is None:
            raise RuntimeError("restore() called without a preceding copy_to()")
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, n_args, *tensors):
        ...
        ctx.n_args = n_args
        ctx.n_params = len(tensors) - n_args
        ctx.rng_state = torch.get_rng_state()
        ctx.save_for_backward(*tensors[:n_args])
        with torch.no_grad():
            out = run_function(*tensors[:n_args])
        return out
    ...
    def backward(ctx, *grad_outputs):
        ...
        detached = [x.detach().requires_grad_(x.requires_grad) for x in inputs]
        ...
        grads = tuple(x.grad if x.requires_grad else None for x in detached)
        return (None, None) + grads + (None,) * ctx.n_params
