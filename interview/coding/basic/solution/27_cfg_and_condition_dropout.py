...
def condition_dropout(cond, null, p, generator):
    ...
    """
    B = cond.shape[0]
    mask = torch.rand(B, generator=generator) < p
    m = mask.reshape(B, *([1] * (cond.dim() - 1)))
    return torch.where(m, null.to(cond.dtype).expand_as(cond), cond), mask
...
def cfg_combine(uncond, cond, scale):
    ...
    if torch.is_tensor(scale) and scale.dim() > 0:
        scale = scale.reshape(-1, *([1] * (cond.dim() - 1))).to(cond.dtype)
...
def cfg_rescale(guided, cond, phi):
    ...
    """
    B = guided.shape[0]
    std_c = cond.reshape(B, -1).std(dim=1, unbiased=False)
    std_g = guided.reshape(B, -1).std(dim=1, unbiased=False)
    s = (std_c / std_g).reshape(B, *([1] * (guided.dim() - 1)))
    return phi * (guided * s) + (1 - phi) * guided
...
def cfg_with_interval(uncond, cond, scale, t, lo, hi):
    ...
    """
    inside = (t >= lo) & (t <= hi)
    w = torch.where(inside, torch.full_like(t, float(scale), dtype=cond.dtype), torch.ones_like(t, dtype=cond.dtype))
    return cfg_combine(uncond, cond, w)
...
def batched_cfg_forward(model, x, t, cond, null, scale):
    ...
    """
    B = x.shape[0]
    c_in = torch.cat([null.to(cond.dtype).expand_as(cond), cond], dim=0)
    out = model(torch.cat([x, x], dim=0), torch.cat([t, t], dim=0), c_in)
    out_uncond, out_cond = out[:B], out[B:]
    return cfg_combine(out_uncond, out_cond, scale)
